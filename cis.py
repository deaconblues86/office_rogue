...
def make_map():
    ...
    while building:
        ...
        if next_row:
            ...
            for hall in h_halls:
                ...
                diff_x = hall.w
                # possible_rooms = room_fill(hall.x1,hall.y2+1,hall.w)
                # possible_rooms = []
                # for r in room_types:
                    # w = len(room_types[r][0]) - 1
                    # h = len(room_types[r]) - 1
                    ...
                    # if w > hall.w and h > hall.w:
                        # continue
                    # elif w > hall.w and h <= hall.w: 
                        # new_room = Rect(x,y,h,w)
                        # flip = libtcod.random_get_int(0, 0, 49)
                        # possible_rooms.append((new_room,r,flip,'man_rotate'))
                    # elif w <= hall.w and h > hall.w:
                ...
                # if possible_rooms != []:
                    ...
                    # print picked_room[1]
                    # print picked_room[0].x1,picked_room[0].y1
                    # print picked_room[0].x2,picked_room[0].y2
                    # print picked_room[0].w,picked_room[0].h
                    # print picked_room[-1]
                    ...
                    # diff_x = hall.x2 - picked_room[0].x2
    ...
    # new_room = Rect(x,y,4,4)
    # print new_room.x1,new_room.x2,new_room.y1,new_room.y2
    # create_hall(new_room)
    
    # for x_cell in range(MAX_ROOMS):
        # x_hall_check = range(MAX_ROOMS)[-1]
        # for y_cell in range(MAX_ROOMS/2):
            # y_hall_check = range(MAX_ROOMS/2)[-1]
            # sect = Rect(x,y,ROOM_SECTOR_X,ROOM_SECTOR_Y)
            # room_sectors.append(sect)
            # y += sect.y2
            
            # if y_hall_check != y_cell:
                # hall_sect = Rect(x,sect.y2,ROOM_SECTOR_X,HALL)
                # hall_sectors.append(hall_sect)
                # y += HALL
        
        # y = OUTSIDE_BORDER
        # x = sect.x2
        # for y_cell in range(MAX_ROOMS/2):
            # if x_hall_check != x_cell:
                # hall_sect = Rect(x,y,HALL,ROOM_SECTOR_Y)
                # hall_sectors.append(hall_sect)
                # y += ROOM_SECTOR_Y
                
                # if y_hall_check != y_cell:
                    # int_sect = Rect(x,y,HALL,HALL)
                    # hall_sectors.append(int_sect)
                    # y += HALL
    ...
    # for h in hall_sectors:
        # print h.x1,h.x2,h.y1,h.y2
        # create_hall(h)
    
    # req_rooms = ['manager','bath','cafe','cube','supply','conf','office','patio']
    
    # rooms = []
    # all_rooms = rooms + hall_sectors
    
    # for r in room_sectors:
        # num_rooms = 0
        # mark_edges(r.edges())
        
        # for int_room in range(4):
            # rindex = libtcod.random_get_int(0, 0, 6)
            # rtype = req_rooms[rindex]
            ...
            # if new_room.x2 > r.x2 or new_room.y2 > r.y2:
                # failed = True
                # print 'outside sector'
                # break
            ...
            # if not failed:
                ...
                # hall_sect = Rect(new_room.x2,new_room.y1,HALL,new_room.h)
                # if hall_sect.x2 < r.x2 or hall_sect.y2 < r.y2:
                    # create_hall(hall_sect)
                    # hall_sectors.append(hall_sect)
                
                # hall_sect = Rect(hall_sect.x1,hall_sect.y2,HALL,HALL)
                # if hall_sect.x2 < r.x2 or hall_sect.y2 < r.y2:
                    # create_hall(hall_sect)
                    # hall_sectors.append(hall_sect)
                
                # hall_sect = Rect(new_room.x1,new_room.y2,new_room.w,HALL)
                # if hall_sect.x2 < r.x2 or hall_sect.y2 < r.y2:
                    # create_hall(hall_sect)
                    # hall_sectors.append(hall_sect)
    ...
    # for r in range(MAX_ROOMS):
        ...
        # new_room = Rect(x, y, w, h)
            ...
            # else:
                ...
                # room_no = Object( 'room_num', new_x, new_y, chr(65+num_rooms), libtcod.white)
                # objects.insert(0, room_no) # Inserting into objects early so others drawn on top
