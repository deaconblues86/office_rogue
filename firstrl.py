...
class Fighter:
    ...
    def attack(self, target):
        ...
        if damage > 0:
            message(self.owner.name.capitalize() + ' attacks ' + target.name + ' for ' + str(damage),libtcod.white)
    ...
    for i in range(num_monsters):
        ...
        if not is_blocked(x,y):
            if libtcod.random_get_int(0,0,100) < 80:
                ...
                monster = Object('orc',x,y,'O',libtcod.desaturated_green,blocks=True,fighter=fighter_component,ai=ai_component)
            else:
                fighter_component = Fighter(hp=16,defense=3,power=4,death_function=monster_death)
                ai_component = BasicMonster()
                
                monster = Object('troll',x,y,'T',libtcod.darker_green,blocks=True,fighter=fighter_component,ai=ai_component)
    ...
    for i in range(num_items):
        ...
        if not is_blocked(x,y):
            ...
            if dice < 70:
                ...
                item = Object('healing potion',x,y,'!',libtcod.violet,item=item_component)
            
            else:
                item_component = Item(use_function=cast_lightning)
                item = Object('scroll of lightning bolt',x,y,'#',libtcod.light_yellow,item=item_component)
...
def cast_lightning():
    ...
    message("A lightning bolt strikes the " + monster.name + " with a loud crack for " + str(LIGHTNING_DAMAGE) + "!", libtcod.light_blue)
    monster.fighter.take_damage(LIGHTNING_DAMAGE)

def closest_monster(max_range):
    closest_enemey = None
    closest_distance = max_range + 1 # Starting further out to grab only those monsters within range (since they'd be one less than this and so at max)
...
# Sets player & NPC
fighter_component = Fighter(hp=30,defense=3,power=5,death_function=player_death)  # Using keyword arguments for clarity

player = Object('player',SCREEN_WIDTH/2,SCREEN_HEIGHT/2,'@',libtcod.white, blocks = True, fighter=fighter_component)
npc = Object('npc',SCREEN_WIDTH/2 - 5,SCREEN_HEIGHT/2,'@',libtcod.yellow, blocks = True)
objects = [npc, player]
