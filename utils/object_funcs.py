...
def mob_death(mob):
    mob.broadcast(mob.name.capitalize() + " is fired!", libtcod.orange)
    mob.char = "%"
...
def toilet_func(target):
    ...
    target.broadcast(target.name.capitalize() + " uses the toilet")
...
def urinal_func(target):
    target.bladder = target.max_bladder
    target.broadcast(target.name.capitalize() + " uses the urinal")
    target.state = "success: " + target.state
    return libtcod.random_get_int(0, 5, 10)  # Wearing out object
...
def terminal_func(target):
    ...
    work_gain = target.max_work * (energy_ratio * 0.3)
    target.work = min(target.work + work_gain, target.max_work)
    target.energy = max(target.energy - (work_gain * 0.25), 0)
    target.broadcast(target.name.capitalize() + " uses their terminal")
    target.state = "success: " + target.state
    return libtcod.random_get_int(0, 5, 10)  # Wearing out object
...
def desk_func(target):
    energy_ratio = float(target.energy) / float(target.max_energy)
    work_gain = target.max_work * (energy_ratio * 0.15)
    target.work = min(target.work + work_gain, target.max_work)
    target.energy = max(target.energy - (work_gain * 0.5), 0)
    target.broadcast(target.name.capitalize() + " uses their desk")
    target.state = "success: " + target.state
    return libtcod.random_get_int(0, 5, 10)  # Wearing out object


def repair_func(worker, target):
    energy_ratio = float(worker.energy) / float(worker.max_energy)
    work_gain = worker.max_work * (energy_ratio * 0.3)
    worker.work = min(worker.work + work_gain, worker.max_work)
    worker.energy = max(worker.energy - (work_gain * 0.25), 0)
    target.broadcast(worker.name.capitalize() + " repairs the " + target.name.capitalize())
...
def coffee_func(target):
    ...
    target.thirst = min(
        target.thirst + (target.max_thirst * 0.25),
        target.max_thirst,
    )
    target.bladder = max(
    ...
    target.broadcast(target.name.capitalize() + " gets some coffee")
    target.state = "success: " + target.state
    return libtcod.random_get_int(0, 5, 10)  # Wearing out object
...
def water_func(target):
    ...
    target.broadcast(target.name.capitalize() + " drinks some water")
    target.state = "success: " + target.state
    return 50
...
def snack_func(target):
    ...
    target.broadcast(target.name.capitalize() + " eats a snack")
    target.state = "success: " + target.state
    return 25
...
def vend_func(target):
    ...
    if target.is_player():
        if target.inventory_full():
            target.broadcast("Your Inventory is full", libtcod.dark_red)
    ...
    if chosen_index == 0 or target.state == "satisfying thirst":
        ...
        #     use_func=water_func,
        # )
        target.broadcast(target.name.capitalize() + " gets a bottle of water")
        ...
        #     use_func=snack_func,
        # )
        target.broadcast(target.name.capitalize() + " gets a snack")
